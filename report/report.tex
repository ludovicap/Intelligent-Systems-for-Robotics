...
\section{Implementazione degli algoritmi di Reinforcement Learning}

Gli algoritmi Q-Learning e SARSA sono stati implementati manualmente in Python, senza utilizzare librerie esterne di Reinforcement Learning o framework predefiniti.

In particolare, sono stati sviluppati direttamente:
\begin{itemize}
    \item la rappresentazione della Q-table;
    \item la politica epsilon-greedy;
    \item le regole di aggiornamento dei valori Q;
    \item il ciclo di training episodico;
    \item la gestione dell’interazione tra agente e ambiente.
\end{itemize}

La Q-table è stata implementata tramite un dizionario Python, nel quale a ogni stato dell’ambiente è associato un vettore contenente i valori Q relativi alle azioni possibili.

L’aggiornamento dei valori Q avviene secondo le equazioni classiche degli algoritmi Q-Learning e SARSA descritte nei capitoli precedenti.
...
\begin{figure}[H]
    ...
    \caption{Confronto della reward media ottenuta dagli agenti durante la fase di valutazione.}
...
\begin{figure}[H]
    ...
    \caption{Confronto del numero medio di cibi raccolti dagli agenti durante gli episodi di test.}
...
\begin{figure}[H]
    ...
    \caption{Confronto del numero medio di step sopravvissuti dagli agenti.}
...
\begin{figure}[H]
    ...
    \caption{Confronto della frequenza di collisione con il fantasma tra gli agenti.}
...
Nel training eseguito, SARSA ottiene prestazioni leggermente superiori in termini di reward media, numero medio di cibi raccolti e collision rate. Tuttavia, la differenza tra i due algoritmi è contenuta, indicando che entrambi riescono ad apprendere politiche efficaci per l’ambiente proposto. Questo comportamento è coerente con la natura off-policy dell’algoritmo, che tende a privilegiare strategie più aggressive e orientate alla massimizzazione della reward futura.
...
In questo progetto è stato sviluppato un ambiente Pac-Man semplificato per studiare l’applicazione di tecniche di Reinforcement Learning basate su Q-table.

Sono stati implementati manualmente gli algoritmi Q-Learning e SARSA, senza utilizzare librerie esterne di Reinforcement Learning. Entrambi gli agenti sono stati addestrati attraverso episodi ripetuti, imparando progressivamente strategie efficaci per raccogliere il cibo ed evitare il fantasma.

I risultati sperimentali mostrano che entrambi gli algoritmi ottengono prestazioni significativamente superiori rispetto a un agente casuale. In particolare, gli agenti addestrati riescono a:
\begin{itemize}
    \item aumentare la reward media;
    \item raccogliere una quantità maggiore di cibo;
    \item sopravvivere per un numero maggiore di step;
    \item ridurre drasticamente il numero di collisioni.
\end{itemize}

Nel training eseguito, SARSA ha ottenuto prestazioni leggermente superiori rispetto a Q-Learning, mostrando un comportamento più stabile e conservativo. Questo risultato è coerente con la natura on-policy dell’algoritmo SARSA, particolarmente adatto ad ambienti parzialmente stocastici.

L’analisi del Q-value iniziale ha mostrato un andamento crescente durante il training, indicando che gli agenti apprendono progressivamente una politica migliore. Tuttavia, il valore non converge perfettamente a una costante, a causa della presenza di esplorazione epsilon-greedy e della componente stocastica dell’ambiente.

Come possibile sviluppo futuro, il progetto potrebbe essere esteso utilizzando tecniche di Deep Reinforcement Learning, come Deep Q-Networks (DQN), per affrontare ambienti più complessi e spazi degli stati di dimensione maggiore.
...
MIT Press, Second Edition, in progress, 2014-2015.
...
\bibitem{course}
Francesco Pupo,
\textit{Lecture Notes of Intelligent Systems for Robotics},
